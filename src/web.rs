...
impl Global {
	pub fn render(&self, template_name: &str, context: Context) -> Response {
		...
		{
			Err(e) => server_error_response(format!(
...
pub fn error_response(status_code: u16, message: String) -> Response {
	Response::builder()
		.status(status_code)
...
pub fn server_error_response(message: String) -> Response { error_response(500, message) }
