...
async fn index(State(g): State<Arc<Global>>) -> Response {
	...
	let identities = match g.api.fetch_my_identities() {
		Ok(i) => i,
		Err(e) => return server_error_response(format!("unable to fetch identities: {}", e)),
	};
...
async fn new_post(
	State(g): State<Arc<Global>>, mut multipart: Multipart,
) -> Response {
    ...
    let description = if description_buf.len() > 0 { Some(FileData {
	...
	match g.api.create_my_identity(
		...
		Ok(_) => Response::builder().status(303).header("Location", "/my-identity").body(Body::empty()).unwrap(),
		Err(e) => server_error_response(format!("Unable to create your new identity: {:?}", e))
	}
}